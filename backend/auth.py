...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if role_requested == "admin":
        ...
        # 프론트엔드가 무조건 client_id='user'로 보내더라도 관리자 계정 로그인을 허용하기 위해 폴백 추가
        if not user:
            admin_fallback = db.query(models.SystemAdmin).filter(models.SystemAdmin.email == form_data.username).first()
            if admin_fallback:
                user = admin_fallback
                role = "admin"
                
        if role == "user" and user and getattr(user, 'deleted_at', None) is not None or getattr(user, 'status', None) == "퇴사" or not getattr(user, 'is_active', True):
            raise HTTPException(
...
def get_current_user_info(credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)):
    try:
        ...
        if role == "admin":
            return {"id": 0, "name": "System Admin", "email": email, "role_name": "시스템 관리자", "role_code": "admin", "sub": email}
        else:
            ...
            if user:
                ...
                return {"id": user.id, "name": user.name, "email": user.email, "role_name": role_desc, "role_code": role_code, "sub": email}
            return {"id": 0, "name": "Unknown", "email": email, "role_name": "Unknown", "role_code": "unknown", "sub": email}
