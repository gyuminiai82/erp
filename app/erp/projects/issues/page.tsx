...
export default function IssuesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${selectedProject}/issues`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.ok ? res.json() : [])
  ...
  return (
    <div className="p-6 bg-gray-50/30 min-h-screen">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">이슈 및 리스크 관리</h1>
          <p className="text-gray-500 mt-2">프로젝트 진행 중 발생한 문제점이나 리스크를 등록하고 해결 과정을 추적합니다.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col h-[calc(100vh-320px)] min-h-[400px] border-2 border-gray-400 shadow-sm overflow-hidden bg-white">
        ...
        <div className="flex-1 overflow-hidden relative">
          {selectedProject ? (<DataGrid columns={columns} data={issues} showCheckboxes={true} selectedRowIndices={selectedRowIndices} onSelectionChange={setSelectedRowIndices} />) : (<div className="flex items-center justify-center h-full text-gray-500">프로젝트를 선택해주세요.</div>)}
        </div>
      </div>
    </div>
  );
