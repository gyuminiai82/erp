...
for filepath, title, desc, btn, varname in files:
    ...
    content = re.sub(r'const \[([a-zA-Z0-9]+), set\1\] = useState\(\[\]\);', r'const [\1, set\1] = useState<any[]>([]);', content, flags=re.IGNORECASE)
    ...
    datagrid_str = f'<DataGrid columns={{columns}} data={{{varname}}} showCheckboxes={{true}} selectedRowIndices={{selectedRowIndices}} onSelectionChange={{setSelectedRowIndices}} />'
    fallback_str = '<div className="flex items-center justify-center h-full text-gray-500">프로젝트를 선택해주세요.</div>'
    
    if varname != 'projects':
        datagrid = f'{{selectedProject ? ({datagrid_str}) : ({fallback_str})}}'
    else:
        datagrid = datagrid_str
