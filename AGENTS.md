...
- **DataGrid UI Pattern**: When using the `DataGrid` component, always match the UI layout pattern from the Employee Management (사원 관리) page. Specifically:
  ...
  2. The `DataGrid` must be wrapped in `<div className="flex flex-col h-[calc(100vh-320px)] min-h-[400px] border-2 border-gray-400 shadow-sm overflow-hidden bg-white">` to ensure a consistent fixed height, prevent full-page scrolling, and apply a thick gray border.
